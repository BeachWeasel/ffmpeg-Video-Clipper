...
function ffnearest() {
  ...
  ffprobe -read_intervals $[$STIME-25]%$[$STIME] -select_streams v -show_frames -show_entries frame=pkt_pts_time,pict_type -v quiet "$1" |
  awk -F= '
...
# Input_Start_vtc_arr=(${Input_Start_vtc//:/ })
...
# Input_End_vtc_arr=(${Input_End_vtc//:/ })
...
then
	...
	then 
		...
		key_before_startvtcsec=$(ffnearest "${vidname}" "${startvtcsec}" "${endvtcsec}")
		...
		#if key_before_startvtcsec-1 <0 then set it to zero
		if [[ $(bc <<<"${key_before_startvtcsec} / 1") -lt 1 ]]
		then
			key_before_startvtcsec=1
		fi
		echo $(bc -l <<< "${key_before_startvtcsec} - 1")
